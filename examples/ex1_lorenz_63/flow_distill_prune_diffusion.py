...
def discover(index, X, Y, Z, params, N, run_idx):
    ...
    fig.savefig(f"./figs/ratios_{index}_diffusion_{run_idx}.png", dpi=200)
    plt.close()

    np.savetxt(f"./outputs/ratios_{index}_diffusion_{run_idx}.txt", np.array(ratios))
...
if __name__ == "__main__":
    ...
    y_data = np.concatenate(
        [
            dx_data[:, 0:1] - sigma * (x_data[:, 1:2] - x_data[:, 0:1]) * dt,
            dx_data[:, 1:2]
            - (x_data[:, 0:1] * (rho - x_data[:, 2:3]) - x_data[:, 1:2]) * dt,
            dx_data[:, 2:3]
            - (x_data[:, 0:1] * x_data[:, 1:2] - beta * x_data[:, 2:3]) * dt,
        ],
        axis=-1,
    )
