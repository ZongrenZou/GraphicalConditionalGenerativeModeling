...
class FMSurrogateEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Skip-lag dynamics input: ancestors are (theta_t, u_t) and (theta_{t-3}, u_{t-3})
        cos_t, sin_t, _ = self._dbuffer[-1]  # theta_t   (most recent)
        cos_tm3, sin_tm3, u_tm3 = self._dbuffer[-4]  # theta_{t-3} (three steps back)

        # FM surrogate input (6-dim): [sin_t, sin_{t-3}, cos_t, cos_{t-3}, u_t, u_{t-3}]
        x = np.array(
            [
                sin_t,
                sin_tm3,
...
if __name__ == "__main__":
    ...
    # Load flow matching surrogate — skip-lag ancestors (theta_t, u_t) and (theta_{t-3}, u_{t-3})
    surrogate = load_fm_surrogate(
        ckpt_dir="./checkpoints/cfm_pruned_2",
    )
    ...
    model = train(
        ...
        seed=42,
        save_dir="./logs/fm_surrogate_skip_2",
    )
