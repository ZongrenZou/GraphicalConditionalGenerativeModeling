...
from ckpt_io import load_model
...
class FMSurrogate:
    ...
    def sample(self, x: np.ndarray) -> float:
        ...
        """
        # Normalize input
        x_norm = jnp.array((x - self.x_mu) / self.x_sd)  # (15,)
...
class FMSurrogateEnv(gym.Env):
    ...
    Dynamics input to GP surrogate (30-dim, W=10):
        [sin(theta_t), sin(theta_{t-1}), ..., sin(theta_{t-9}),
         cos(theta_t), cos(theta_{t-1}), ..., cos(theta_{t-9}),
         u_t, u_{t-1}, ..., u_{t-9}]
    ...
    def __init__(
        ...
        self._dbuffer = (
            None  # deque of (cos, sin, last_u) frames, maxlen=10 (dynamics input)
        )
    ...
    def step(self, action):
        ...
        # W=10 dynamics input from _dbuffer (oldest ... newest)
        df = list(self._dbuffer)  # 10 frames, df[-1] is most recent (theta_t)
        cos_t, sin_t, _ = df[-1]
        cos_tm1, sin_tm1, u_tm1 = df[-2]
        cos_tm2, sin_tm2, u_tm2 = df[-3]
        cos_tm3, sin_tm3, u_tm3 = df[-4]
        cos_tm4, sin_tm4, u_tm4 = df[-5]
        cos_tm5, sin_tm5, u_tm5 = df[-6]
        cos_tm6, sin_tm6, u_tm6 = df[-7]
        cos_tm7, sin_tm7, u_tm7 = df[-8]
        cos_tm8, sin_tm8, u_tm8 = df[-9]
        cos_tm9, sin_tm9, u_tm9 = df[-10]

        # FM surrogate input (W=10, 30-dim):
        # [sin_t, ..., sin_{t-9}, cos_t, ..., cos_{t-9}, u_t, ..., u_{t-9}]
        x = np.array(
            [
                sin_t,
                sin_tm1,
        ...
        self._dbuffer.append(new_frame.copy())  # W=10 dynamics buffer
...
if __name__ == "__main__":
    ...
    # Load flow matching surrogate directly from checkpoint
    surrogate = load_fm_surrogate(
        ckpt_dir="./checkpoints/cfm_all",
    )

    # Train SAC in FM surrogate environment (W=3 agent obs, W=10 dynamics)
    model = train(
        surrogate=surrogate,
        max_torque=2.0,
        total_timesteps=200_000,
        seed=42,
        save_dir="./logs/fm_surrogate_w10",
    )
